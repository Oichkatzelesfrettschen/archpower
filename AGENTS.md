# archpower Agent Guide

This file holds the rules and orientation for every agent and contributor working in this repository.

## Instruction source

`AGENTS.md` is the root instruction file for archpower and owns its rules. Every agent and contributor reads it directly. `CLAUDE.md` is a tracked, repository-relative symbolic link to `AGENTS.md`, so Claude Code receives the canonical rules through the same bytes and the body lives in one place. A tool that requires a differently named loader references this file rather than copying doctrine that can drift.
